...
def robot_transformation_matrix(theta_1: float = 0, theta_2: float = 0,
                                theta_3: float = 0, theta_4: float = 0,
                                theta_5: float = 0, theta_6: float = 0):
    theta_1 = np.radians(theta_1) * pi / 180  # [rad]
    theta_2 = np.radians(theta_2) * pi / 180   # [rad]
    theta_3 = np.radians(theta_3) * pi / 180   # [rad]
    theta_4 = np.radians(theta_4) * pi / 180   # [rad]
    theta_5 = np.radians(theta_5) * pi / 180   # [rad]
    theta_6 = np.radians(theta_6) * pi / 180   # [rad]
    theta_7 = -pi / 2  # [rad]
...
def gripper_to_base(transform_matrix):
    rotation_matrix = transform_matrix[:3, :3]
    print('rotation matrix:', rotation_matrix)

    r = cv2.Rodrigues(rotation_matrix)[0]
